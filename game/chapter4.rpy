...
label chapter4:
    ...
    p "드디어 축제 마지막 꽃인 콘서트다. 대학 축제를 마무리하는 대규모 콘서트가 열린다는데..."
    p "누구와 함께 이 특별한 밤을 보낼지 고민된다. 아니면 그냥 혼자 즐길까?"
...
label concert_with_chanmi:
    ...
    p "찬미가 나를 보며 활짝 웃는 모습에 나도 모르게 미소가 번진다. 이 순간은 정말 잊지 못할 것 같다."
    c "봐봐, 다음 곡이 내가 제일 좋아하는 노래야. 이 노래 나올 때 다들 엄청난 에너지를 보여줘. 나랑 같이 뛰자!"
    c "약속해. 오늘 이 밤은 우리 둘만의 기억으로 남길 거야."
    p "그녀와 함께 춤추며 밤이 깊어간다. 그리고 우리는 축제의 마지막 밤을 누구보다도 뜨겁게 즐겼다."
    return
...
label concert_with_sena:
    ...
    p "세나는 조용한 목소리로 말하면서도 눈에는 설렘이 가득 담겨 있었다."
    s "오늘 하루, 정말 특별했으면 좋겠어. 너와 함께하는 마지막 순간이 이렇게 아름답게 기억될 수 있도록..."
    p "그녀의 말에 무심코 고개를 끄덕였다. 세나와 함께라면 그 어떤 노래도 특별할 것 같다."
    s "저기, 다음 곡은 내가 정말 좋아하는 곡이야. 가사도 멜로디도 내 마음을 꼭 닮은 것 같아서 들을 때마다 마음이 흔들리곤 해."
    s "너랑 같이 들으면 더 특별할 것 같아. 그냥... 너랑 공유하고 싶은 느낌이야."
    p "공연이 끝나갈 무렵, 세나는 내 쪽으로 고개를 돌려 작은 목소리로 말했다."
    s "오늘의 이 감정... 절대 잊지 않을 거야. 너도, 나도."
    p "그녀의 손길이 살짝 닿은 순간, 이 밤이 우리의 마음 속 깊이 새겨질 것임을 느낄 수 있었다."
    return

label concert_with_aricet:
    scene bg_concert with fade
    a "음, 생각보다 시끄럽지는 않네. 나쁘지 않아."
    p "아리셋은 평소와 같은 차분한 목소리였지만, 약간 긴장한 듯 보였다."
    ...
    p "그녀는 말끝마다 조금씩 망설였지만, 그 진심은 분명히 전해졌다."
    a "아, 저 곡. 저 곡은 가사가 정말 아름다워. 들을 때마다 가슴 깊이 울려. 너도 들어봤으면 좋겠다."
    a "너도 그 감정을 느꼈으면 좋겠어. 이런 걸 나 혼자 느끼는 건 아깝잖아."
    p "아리셋의 눈빛이 반짝였다. 그녀가 음악에 몰입하는 모습을 보며 나도 점점 빠져들었다."
    a "오늘의 이 시간은... 나에게 정말 의미 있는 순간이 될 거야. 네가 있어서 가능했어. 고마워."
    p "그녀와 함께 조용히 공연장을 빠져나오며, 나는 그녀가 말하지 않아도 전해지는 감정을 느꼈다."
    return

label solo_ending:
    scene bg_concert with fade
    p "혼자 콘서트를 보러 온 건 처음이었다. 공연장은 활기찼지만, 나의 마음은 조금 공허했다."
    p "사람들은 저마다 소중한 사람과 함께 웃고, 떠들고, 노래를 즐기고 있었다. 그런 모습을 보니 조금 부럽기도 했다."
    p "하지만 이 밤, 이 음악을 혼자라도 온전히 즐기기로 했다. 축제의 마지막 밤은 나만의 방식으로 특별하게 만들어야지."
    p "마지막 곡이 울려 퍼질 때, 나는 문득 고개를 들어 밤하늘을 바라봤다. 별빛이 반짝이는 하늘 아래, 나 혼자지만 외롭지 않았다."
    p "이 순간은 내 자신에게 주는 선물이다. 그리고 다음에는, 이 축제를 함께 나눌 누군가를 찾을 수 있을지도 모른다."
    return

# 배경 전환 효과와 기본 이미지를 설정
image bg_concert = "concert_background.jpg"
